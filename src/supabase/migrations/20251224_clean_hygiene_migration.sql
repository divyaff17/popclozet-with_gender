-- ============================================
-- CLEAN HYGIENE SYSTEM MIGRATION
...
DROP POLICY IF EXISTS "Anyone can manage products for testing" ON products;
...
DROP POLICY IF EXISTS "Anyone can view hygiene SOPs for testing" ON hygiene_sops;
DROP POLICY IF EXISTS "Anyone can insert hygiene SOPs for testing" ON hygiene_sops;
DROP POLICY IF EXISTS "Anyone can update hygiene SOPs for testing" ON hygiene_sops;
...
DROP POLICY IF EXISTS "Anyone can view QR codes for testing" ON product_qr_codes;
DROP POLICY IF EXISTS "Anyone can manage QR codes for testing" ON product_qr_codes;
DROP POLICY IF EXISTS "Users can view their own role" ON user_roles;
DROP POLICY IF EXISTS "Admin can manage all roles" ON user_roles;
DROP POLICY IF EXISTS "Anyone can view roles for testing" ON user_roles;
DROP POLICY IF EXISTS "Anyone can manage roles for testing" ON user_roles;
...
-- Add hygiene columns to products (IF NOT EXISTS for each column)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='fabric_type') THEN
        ALTER TABLE products ADD COLUMN fabric_type TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='fabric_hint') THEN
        ALTER TABLE products ADD COLUMN fabric_hint TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='hygiene_sop_id') THEN
        ALTER TABLE products ADD COLUMN hygiene_sop_id UUID REFERENCES hygiene_sops(id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='rental_count') THEN
        ALTER TABLE products ADD COLUMN rental_count INTEGER DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='last_cleaned_at') THEN
        ALTER TABLE products ADD COLUMN last_cleaned_at TIMESTAMP WITH TIME ZONE;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='condition_status') THEN
        ALTER TABLE products ADD COLUMN condition_status TEXT DEFAULT 'excellent' CHECK (condition_status IN ('excellent', 'good', 'fair', 'needs_repair'));
    END IF;
END $$;
...
-- Create policies for testing (allow all access)
...
DO $$
BEGIN
    RAISE NOTICE '✅ Hygiene system migration completed successfully!';
    RAISE NOTICE '✅ All tables, indexes, and policies are ready';
    RAISE NOTICE '📝 Testing mode enabled - full access granted';
END $$;
